...
for idx, bench in enumerate(benches):
    ...
    file.write("\t\t<command>\n")
    file.write("\t\t\t$(JAVA_COMMAND) $(ADD_OPENS_CMD) $(JVM_OPTIONS) -XX:+UseG1GC -Xms12G -Xmx12G -XX:ThreadPriorityPolicy=1 -XX:+AlwaysPreTouch -XX:+UseLargePages -XX:+UseTransparentHugePages")
    file.write(" -jar $(Q)$(TEST_RESROOT)$(D)renaissance-jmh.jar$(Q) -f " + str(forks[idx]) + " -bm avgt -rf json -rff $(Q)$(REPORTDIR)$(D)" + bench + ".json$(Q) " + bench + "; $(TEST_STATUS)\n")
    file.write("\t\t</command>\n")
